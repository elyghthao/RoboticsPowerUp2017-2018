    ...
    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=PARAMETERS
    	if (AutonomousCommand.position.equals("L")) { //if on the left side of the alliance wall aka POSITION 1
    		if (Robot.Debug ) {
    			...
    			System.out.println("AutoRun Left position");
    		}
    		addSequential(new DriveStraight(0.5, 1.0, 20.0)); //move forward to about 20 feet (will do math later), or move for 8.0 seconds. Should be adjacent to platform zone
    		Timer.delay(0.2);
    		addSequential(new Pivot(90.0)); //turn 90 degrees to the right to face platform zone
    		Timer.delay(0.2);
    		addSequential(new DriveStraight(0.5, 1.0, 20.0)); //drive straight for about 5 feet or 2 seconds to be in platform zone and out of everyone's way
    	}
    	
    	
    	if (AutonomousCommand.position.equals("M")) { //if in POSITION 2 aka middle position
    		addSequential(new DriveStraight(0.8, 0.5, 5.0));//drive forward slightly
    		if (AutonomousCommand.gameData.substring(0,1).equals("L")) { //if the alliance switch is on the left side
    			addSequential(new Pivot(50.0)); //after moving forward slightly, turn 50 degrees to the right
    			Timer.delay(0.2);
    			...
    			Timer.delay(0.2);
    			addSequential(new Pivot(-50.0)); //turn left 40 degrees
    			Timer.delay(0.2);
    			addSequential(new DriveStraight(0.8, 5.0, 12.0)); // move forward to be adjacent to platform zone
    			Timer.delay(0.2);
    			addSequential(new Pivot(-90.0)); //turn left 90 degrees to be face the platform zone
    		}
    		else if (AutonomousCommand.gameData.substring(0,1).equals("R")) {
    			addSequential(new Pivot(-50.0)); //after moving forward, turn left 50 degrees
    			Timer.delay(0.2);
    			addSequential(new newDriveStraight(0.8, 4.0, 12.0)); // move forward to clear the switch and have a clear path to the right
    			Timer.delay(0.2);
    			addSequential(new Pivot(-50.0)); //turn right 40 degrees
    			Timer.delay(0.2);
    			addSequential(new DriveStraight(0.8, 5.0, 12.0)); //move forward to be adjacent to platform zone
    			Timer.delay(0.2);
    			addSequential(new Pivot(90.0)); //turn right 90 degrees to face platform zone
    		}
    		Timer.delay(0.2);
    	...
    	if (AutonomousCommand.position.equals("R")) { // if on right side of the alliance wall aka POSITION 3
    		...
    		Timer.delay(0.2);
    		addSequential(new Pivot(-90.0)); // turn left 90 degrees to face the PLATFORM ZONE
    		Timer.delay(0.2);
